...
const itens = [
  ...
  // Massas
  // ... (mantém aqui o resto dos itens que você já tinha no arquivo original)
];
...
function renderMenu(filter = '') {
  ...
  // agrupa itens por categoria, já aplicando filtro
  const grouped = itens.reduce((map, item) => {
    const text = (
      item.namePt +
      item.nameEn +
      item.cat +
    ...
    if (term && !text.includes(term)) return map;
  ...
  Object.entries(grouped).forEach(([cat, items]) => {
    ...
    heading.className = 'menu-box-title';
    ...
    items.forEach((i) => {
      ...
      title.innerHTML = `<span>${i.namePt}</span><span class="titleEN">${i.nameEn}</span>`;

      const price = document.createElement('div');
      price.className = 'menu-price';
      price.textContent = currency(i.price);
      ...
      minus.type = 'button';
      minus.className = 'stepper-btn stepper-minus';
      minus.textContent = '−';
      ...
      input.value = String(qty.get(i.namePt) || 0);

      const plus = document.createElement('button');
      plus.type = 'button';
      plus.className = 'stepper-btn stepper-plus';
      plus.textContent = '+';
      ...
      desc.className = 'desc';
      desc.innerHTML = `<p>${i.descPt}</p><p class="titleEN">${i.descEn}</p>`;
      ...
      function sync() {
        qty.set(i.namePt, Number(input.value || 0));
        updateTotal();
        desc.style.display =
          Number(input.value || 0) > 0 ? 'block' : 'none';
      }
      ...
      row.addEventListener('click', (ev) => {
        if (ev.target === minus || ev.target === plus || ev.target === input)
          return;
        desc.style.display =
          desc.style.display === 'block' ? 'none' : 'block';
      });
...
function buildMessage({ quarto, hora, nome, sobrenome, obs }) {
  ...
  linhas.push(
    'Gostaria de fazer um pedido de serviço de quarto. / I would like to place a room service order.',
  );
  ...
  if (sel.length) {
    ...
    sel.forEach((i) => {
      const q = qty.get(i.namePt) || 0;
      linhas.push(
        `• ${q}x ${i.namePt} (${i.nameEn}) – ${currency(i.price)}`,
      );
    });
    const total = sel.reduce(
      (acc, i) => acc + (qty.get(i.namePt) || 0) * i.price,
      0,
    );
    linhas.push(
      `Total estimado • Estimated total: ${currency(total)}`,
    );
  }
...
function openWhatsApp(text) {
  const url = `https://wa.me/${DEFAULT_PHONE}?text=${encodeURIComponent(
    text,
  )}`;
...
function toggleFloatingButton() {
  ...
  const qty = window.__qty || new Map();
  const hasItems = Array.from(qty.values()).some(
...
if (manutencaoForm) {
  manutencaoForm.addEventListener('submit', (e) => {
    ...
    const issues = Array.from(
      manutencaoForm.querySelectorAll(
    ...
    const linhas = [
      ...
      issues.length
        ? `Problemas selecionados • Selected issues: ${issues.join(', ')}`
        : '',
...
const canalLinhas = Array.from(
  document.querySelectorAll('.channels-table tbody tr'),
);
